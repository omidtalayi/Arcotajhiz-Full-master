CREATE PROCEDURE [AZ].[CancelPayIdentityInformation](
...
AS BEGIN
    ...
    BEGIN TRY
		...
		BEGIN
			SELECT @IdentityInformationVCode = IdentityInformationVCode FROM AZ.OnlinePayment WHERE VCode = @OnlinePaymentID
			If @IdentityInformationVCode IS NULL 
			BEGIN
				RAISERROR('"OnlinePaymentID" is invalid',16,126)
			END

			IF EXISTS(SELECT 1 FROM AZ.IdentityInformation WHERE VCode = @IdentityInformationVCode AND IdentityInformationStateVCode = 1)
			BEGIN
