CREATE PROCEDURE [AZ].[CancelPayIdentification](
...
AS BEGIN
    ...
    BEGIN TRY
		...
		BEGIN
			SELECT @IdentificationVCode = IdentificationVCode FROM AZ.OnlinePayment WHERE VCode = @OnlinePaymentID
		...
		BEGIN
			IF EXISTS(SELECT 1 FROM AZ.Accounting WHERE OnlinePaymentVCode = @OnlinePaymentID)
			BEGIN
				DELETE AZ.Accounting WHERE OnlinePaymentVCode = @OnlinePaymentID
			END
			SET @State = 1
			COMMIT TRAN
		END
