CREATE PROCEDURE [AZ].ReceivedPayCompanyPerson(
	...
	@CompanyPersonVCode BIGINT OUTPUT,
...
AS BEGIN
    BEGIN TRY
		...
		BEGIN
			SELECT @CompanyPersonVCode = CompanyPersonVCode, @PaymentAmount = Amount FROM AZ.OnlinePayment WHERE VCode = @OnlinePaymentID
			If @CompanyPersonVCode IS NULL 
				RAISERROR('"OnlinePaymentID" is invalid',16,126)

			IF NOT EXISTS(SELECT 1 FROM AZ.CompanyPerson WHERE VCode = @CompanyPersonVCode AND CompanyPersonStateVCode = 1) 
			BEGIN
				RAISERROR('CompanyPerson has an invalid state for completing the payment',16,126)
			END
		END
