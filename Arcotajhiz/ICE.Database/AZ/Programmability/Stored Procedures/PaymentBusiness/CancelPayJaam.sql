CREATE PROCEDURE [AZ].[CancelPayJaam](
...
AS BEGIN
    ...
    BEGIN TRY
		...
		BEGIN
			SELECT @JaamVCode = JaamVCode FROM AZ.OnlinePayment WHERE VCode = @OnlinePaymentID
			If @JaamVCode IS NULL 
			BEGIN
				RAISERROR('"OnlinePaymentID" is invalid',16,126)
			END

			IF EXISTS(SELECT 1 FROM AZ.Jaam WHERE VCode = @JaamVCode AND JaamStateVCode = 3)
			BEGIN
