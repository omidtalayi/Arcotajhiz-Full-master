CREATE PROCEDURE [AZ].[ReceivedPayGroupOfPersons](
...
AS BEGIN
    BEGIN TRY
		...
		BEGIN
			SELECT @GroupOfPersonsVCode = GroupOfPersonsVCode, @PaymentAmount = Amount FROM AZ.OnlinePayment WHERE VCode = @OnlinePaymentID
			If @GroupOfPersonsVCode IS NULL 
				RAISERROR('"OnlinePaymentID" is invalid',16,126)

			IF NOT EXISTS(SELECT 1 FROM AZ.GroupOfPersons WHERE VCode = @GroupOfPersonsVCode AND GroupOfPersonsStateVCode = 1) 
			BEGIN
				RAISERROR('GroupOfPersons has an invalid state for completing the payment',16,126)
			END
		END
