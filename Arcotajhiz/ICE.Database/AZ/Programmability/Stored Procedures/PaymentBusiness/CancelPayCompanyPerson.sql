CREATE PROCEDURE [AZ].[CancelPayCompanyPerson](
...
AS BEGIN
    ...
    BEGIN TRY
		...
		BEGIN
			SELECT @CompanyPersonVCode = CompanyPersonVCode FROM AZ.OnlinePayment WHERE VCode = @OnlinePaymentID
			If @CompanyPersonVCode IS NULL 
			BEGIN
				RAISERROR('"OnlinePaymentID" is invalid',16,126)
			END

			IF EXISTS(SELECT 1 FROM AZ.CompanyPerson WHERE VCode = @CompanyPersonVCode AND CompanyPersonStateVCode = 1)
			BEGIN
