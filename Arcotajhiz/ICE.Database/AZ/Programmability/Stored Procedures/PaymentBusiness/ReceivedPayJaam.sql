CREATE PROCEDURE [AZ].[ReceivedPayJaam](
...
AS BEGIN
    BEGIN TRY
		...
		BEGIN
			SELECT @JaamVCode = JaamVCode, @PaymentAmount = Amount FROM AZ.OnlinePayment WHERE VCode = @OnlinePaymentID
			If @JaamVCode IS NULL 
				RAISERROR('"OnlinePaymentID" is invalid',16,126)

			IF NOT EXISTS(SELECT 1 FROM AZ.Jaam WHERE VCode = @JaamVCode AND JaamStateVCode = 3) 
			BEGIN
				RAISERROR('Jaam has an invalid state for completing the payment',16,126)
			END
		END
